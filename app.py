#!/usr/bin/env python3
"""
Dating Chat - Web Application
Applicazione web completa per chat di incontri.
"""
...
SECRET_KEY = __import__("os").environ.get("SECRET_KEY", secrets.token_hex(32))  # In produzione usa variabile d'ambiente

app = FastAPI(title="Dating Chat")
...
async def register(
    ...
    conn = get_db()
    try:
        # Usa lo stesso hash dello script originale per compatibilità, oppure bcrypt
        # Per semplicità e sicurezza usiamo bcrypt
...
async def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if user and verify_password(password, user["password_hash"]):
        ...
        # Aggiorna online
        conn = get_db()
        conn.execute("UPDATE users SET is_online = 1, ultimo_accesso = CURRENT_TIMESTAMP WHERE id = ?", (user["id"],))
        conn.commit()
        conn.close()
        return RedirectResponse("/discover", status_code=303)
...
async def discover(request: Request):
    ...
    conn = get_db()
    # Trova utenti non ancora swipati, non se stessi, non bloccati
...
async def do_swipe(request: Request, to_user_id: int = Form(...), tipo: str = Form(...)):
    ...
    try:
        conn.execute("""
            INSERT INTO swipes (from_user_id, to_user_id, tipo)
            VALUES (?, ?, ?)
        """, (user["id"], to_user_id, tipo))

        match_created = False
        if tipo in ("like", "superlike"):
            # Controlla reciprocità
...
async def matches_page(request: Request):
    ...
    for m in matches:
        d = dict(m)
        d["eta"] = calcola_eta(conn.execute("SELECT data_nascita FROM users WHERE id = ?", (d["altro_id"],)).fetchone()["data_nascita"])
        match_list.append(d)
...
async def chat_page(request: Request, conversation_id: int):
    ...
    # Segna come letti
    conn.execute("""
        UPDATE messages SET letto = 1
        WHERE conversation_id = ? AND sender_id != ? AND letto = 0
...
async def update_interessi(request: Request):
    ...
    for iid in selected:
        try:
            conn.execute("INSERT INTO user_interests (user_id, interest_id) VALUES (?, ?)", (user["id"], int(iid)))
        except:
            pass
    conn.commit()
    conn.close()
